...
function Unit:update(dt)
	...
	if #food > 0 then
		...
		for i,v in ipairs(food) do
			local distx = math.abs(food[i].x - self.x)
			local disty = math.abs(food[i].y - self.y)
			self.distance[i] = math.sqrt((distx*distx)+(disty*disty))
		end
	
		min = returnMin(self.distance)

		angle = math.atan2(food[min].y - self.y, food[min].x - self.x)
	    cos = math.cos(angle)
	    sin = math.sin(angle)

	    if self.distance[min] <= self.sight.radius+food[min].radius then
			self.x = self.x + self.speed * cos * dt
	    	self.y = self.y + self.speed * sin * dt
			if self.distance[min] < food[min].radius then
				table.remove(food,min)
			end
		end
	end
   ...
   --[[ for i=0,#food do
    ...
    --[[if self.distance[0] > self.distance[1] then
    	if self.distance[0] <= self.sight.radius+food[0].radius then
			self.x = self.x + self.speed * cos * dt
	    	self.y = self.y + self.speed * sin * dt
		end
    else
    	if self.distance[1] <= self.sight.radius+food[1].radius then
			self.x = self.x + self.speed * cos * dt
	    	self.y = self.y + self.speed * sin * dt
		end
    end]]
...
function Unit:draw()
	...
	for i,v in ipairs(food) do
		if self.distance[i] <= self.sight.radius+food[i].radius then
			love.graphics.print(food[i].str .. ' YES / ' .. math.floor(self.distance[i]+0.5), 0, small_height*(i-1))
		--elseif self.distance[i] == self.sight.radius+food[i].radius then
		--	love.graphics.print(i ..' == / ' .. self.distance[i], 0, small_height*i)
		elseif self.distance[i] > self.sight.radius+food[i].radius then
			love.graphics.print(food[i].str ..' NO / ' .. math.floor(self.distance[i]+0.5), 0, small_height*(i-1))
		end

		--love.graphics.print(food[min].str, 0, window_height-small_height*2)
	end

	--[[if #food > 0 then
		love.graphics.print(#food, 0, window_height-small_height*4)
		love.graphics.print(min, 0, window_height-small_height*3)
		love.graphics.print(food[min].str, 0, window_height-small_height*2)
	end]]
	...
	--[[if self.distance[0] <= self.sight.radius+food[0].radius then
		love.graphics.print(0 .. ' YES', 0, small_height*3)
	else
		love.graphics.print(0 ..' NO', 0, small_height*3)
	end

	if self.distance[1] <= self.sight.radius+food[1].radius then
		love.graphics.print(1 .. ' YES', 0, small_height*4)
	else
		love.graphics.print(1 ..' NO', 0, small_height*4)
	end]]
	...
	--[[for i=0,#food do
		...
		y_food[i] = food[i].y
		love.graphics.setColor(255,255,255)
		love.graphics.line( x1, y1, x_food[i], y_food[i])
	end]]

	--love.graphics.print('D: ' .. self.distance[0] * temp_dt , 0, small_height*6)
	--love.graphics.print('D: ' .. self.distance[1] * temp_dt , 0, small_height*7)
